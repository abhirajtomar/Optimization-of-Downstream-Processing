...
public class step_5 extends JFrame implements ActionListener{
	...
	double Rev=0;
	double sp=0;
public step_5(double product_in,double loadconc,double tt,double inprod,double SP) {
		
		this.setSize(750,250);
		setTitle("Process: Polishing");
	...
	sp=SP;
	 ...
	 public void actionPerformed(ActionEvent evente)
     {
		 ...
		 out.println("***Input for Polishing Process***");
		...
		SY = SY/100;	 
		 ...
		 System.out.println("RV=" +RVu);
			...
			TT = TT + total_time+168;
			product= product*SY;
			TPY = (product/inproduct)*100;
			BPY = (365*24)/TT;
			total_prod = BPY*product;
			Rev= total_prod*sp;
			 System.out.println("Optimum Time=" +total_time);
			 if(flag==0)
			 {
			 new step5_out(oRV,onoc,oeq4cv,oloading,owashing,oelution,oreg2cv,product,total_time,TT,TPY,BPY,total_prod,Rev);
			 }
			 else
			 {
				 new Invalid();
